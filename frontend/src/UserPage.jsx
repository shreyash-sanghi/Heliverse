...
useEffect(()=>{
    ...
    <nav class="relative px-4 py-4 flex flex-col sm:flex-row justify-between bg-blue-900 items-center ">
		...
		<Link to={"/postlist"}class="text-lg sm:text-3xl text-white	  font-bold leading-none" href="#">
         ...
         <div>  
        {(iniName=="")?(<>
		<Link to={"/login"} class="hidden mr-1 sm:ml-auto sm:mr-3 px-1 py-2 xsm:px-2 sm:px-6 bg-gray-50 hover:bg-gray-100 text-sm text-gray-900 font-bold  rounded-xl transition duration-200" href="#">{iniName}</Link>
		</>):(<>
		<Link to={"/myprofile"} class="sm:inline-block mr-1 xsm:mr-2 px-1 sm:ml-auto sm:mr-3 py-2 xsm:px-2 sm:px-6 bg-gray-50 hover:bg-gray-100 text-sm text-gray-900 font-bold  rounded-xl transition duration-200" href="#">{iniName}</Link>
		</>)}
		<button onClick={()=>{
             navigate("/createteam")
		}} class="sm:inline-block py-2 px-1 xsm:px-2 sm:px-6 sm:mr-3  bg-blue-500 hover:bg-blue-600 text-sm text-white font-bold rounded-xl transition duration-200" href="#">Create a team</button>
         		<button onClick={()=>{
             navigate("/myteam")
		}} class="sm:inline-block py-2 px-1 xsm:px-2 sm:px-3 bg-green-200 hover:bg-blue-400 text-sm text-black font-bold rounded-xl transition duration-200" href="#">My team</button>
